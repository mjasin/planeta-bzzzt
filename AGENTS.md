...
- Whenever executing shell commands in the project workspace (e.g. `git`, `ls`, `diff`, `grep`, `find`), **always prefix supported commands with `rtk`** (e.g. `rtk git status`, `rtk git diff`, `rtk ls`).
- `rtk` optimizes and filters terminal output to minimize token usage in the LLM context.
- Dedicated rules file: `.agents/rules/antigravity-rtk-rules.md`.
